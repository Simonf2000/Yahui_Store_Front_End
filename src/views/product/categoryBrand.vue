<template>
    ...
    <el-pagination v-model:current-page="pageParams.page" v-model:page-size="pageParams.limit" :page-sizes="[2, 5, 10, 20]"
...
const saveOrUpdate = () => {
    //进行表单数据校验，表单数据必填
    if (typeof categoryBrand.value.brandId == 'undefined' || categoryBrand.value.brandId == '') {
        ElMessage.info('品牌信息必须选择')
        return
    }
    //categoryId为数组：[1,2,3]
    if (typeof categoryBrand.value.categoryId == 'undefined' || categoryBrand.value.categoryId.length != 3) {
